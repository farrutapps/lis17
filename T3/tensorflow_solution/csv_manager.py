...
## return value: None
  def save_to_file(self,filename, data, header=None, types=None):
    ...
    if types:
      np.savetxt(file,data, delimiter=',', fmt=types)
...
# data: np.array containing the content of the file.
  ...
  def restore_from_hdf5(self,filename):
    data_pandas = pd.read_hdf(self._filepath(filename), filename[:-3])
    data_numpy = data_pandas.as_matrix()

    ids = data_pandas.index.values.reshape(data_numpy.shape[0],1)

    data_numpy = np.hstack([ids,data_numpy])
...
### TESTING
#man = CsvManager('/Users/Sebastian/Programming/ETHProg/LIS/T3/data')
#data = man.restore_from_hdf5('test.h5')
#print data
